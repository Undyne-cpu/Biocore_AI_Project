...
除公开接口外，所有接口都需要在请求头中携带 Token：

```
Authorization: Bearer <token>
```

Token 获取方式：登录成功后返回 `token` 字段。
...
```json
{
  ...
  "data": {
    ...
    "version": "4.4.0.0",
    "parameters": [
      {"name": "threads", "label": "线程数", "type": "number", "default": 4}
    ],
...
```json
{
  "name": "string (required)",
  "description": "string (optional)",
  "nodes": [{"id": "node1", "tool": "fastqc", "name": "FastQC"}],
  "edges": [{"source": "node1", "target": "node2"}],
  "settings": {"parallelTasks": 4}
}
```
...
```json
{
  "inputFiles": {"start": "file_001"},
  "settings": {"saveCheckpoints": true}
}
```
...
```
1. 用户注册/登录 → 获取 token
2. 所有请求携带: Authorization: Bearer <token>
3. Token 过期 → 调用 /auth/refresh 刷新
```
